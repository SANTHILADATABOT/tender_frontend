...
const CompetitorBranch = () => {
  ...
  const getBranchList = async () => {
    var dataset;
    const db_branchlist=await axios.get(`${baseUrl}/api/competitorbranch`);
      if(db_branchlist.data.status===200){
        // setBranchList(db_branchlist.data.branch);
      ...
      $("#dataTable tbody").on("click", "tr .fa-edit", function () {
        ...
        // navigate(
        //   `/tender/master//competitorcreation/competitor/details/branch/${rowdata.id}`
        // );
      });
  ...
  const getBranchDetails = async (rowid) => {
    ...
    let fetchedData = await axios.get(`${baseUrl}/api/competitorbranch/${rowid}`);
    setEditableRow((prev)=>{return {...prev,
    ...
    setCompetitorBranchInput((prev)=>{return {...prev,
  ...
  useEffect(()=>{ 
    if(editableRow.branchId && editableRow.country !=="" && countryList.length >0)
    {
    setCompetitorBranchInput({ ...competitorBranchInput, country: countryList.find((x)=>x.value === editableRow.country)
    })}
  ...
  const getCompNo = async () => {
    await axios.get(`${baseUrl}/api/competitorprofile/getcompno/${compid}`).then((resp) => {
      if(resp.data.status===200){
        setCompetitorBranchInput({...competitorBranchInput,compNo:resp.data.compNo});
      }
    });
  };
  ...
  const getStateList = async () => {
    await axios.get(`${baseUrl}/api/state/list/${competitorBranchInput.country.value}`).then((resp) => {
      setStateList(resp.data.stateList);
      setDistrictList([]);
    });
  };
  const getDistrictList = async () => {
    await axios.get(`${baseUrl}/api/district/list/${competitorBranchInput.country.value}/${competitorBranchInput.state.value}`).then((resp) => {
      setDistrictList(resp.data.districtList);
      setCityList([]);
    });
  };
  const getCityList = async () => {
    await axios.get(`${baseUrl}/api/city/list/${competitorBranchInput.country.value}/${competitorBranchInput.state.value}/${competitorBranchInput.district.value}/null`).then((resp) => {
...
useEffect(()=>{
  console.log()
    if(!competitorBranchInput.branchId>0 && (competitorBranchInput.country!==null) && countryList.length>0)
    {
      console.log("IN 251");
      getStateList();
      setDistrictList([]);
    ...
    else if(competitorBranchInput.branchId>0 && (competitorBranchInput.country!==null) && countryList.length>0){
      console.log("IN 256");
      getStateList();
        setCompetitorBranchInput((prev)=>{return { ...prev, state: stateList.find((x)=>x.value === editableRow.state),district:null, city:null}});
      setCityList([]);
    }
    else{
      console.log("IN 263");
        setCompetitorBranchInput((prev)=>{return {...prev, state:null,district:null, city:null}});
        setStateList([]);
        setDistrictList([]);
        setCityList([]);
    }
...
useEffect(()=>{
    ...
    {getDistrictList();}
        setCompetitorBranchInput((prev)=>{return {...prev, district:null, city:null}});
        setDistrictList([]);
        setCityList([]);
...
useEffect(()=>{
    ...
    {getCityList();}
        setCompetitorBranchInput((prev)=>{return {...prev, city:null}});
        setCityList([]);
},[competitorBranchInput.district]);

useEffect(()=>{
    if(hasError.country===false && hasError.state===false && hasError.district===false && hasError.city===false)
    {
        setFormIsValid(true);
    }
},[hasError]);
  ...
  const selectInputHandler = (value, action) => {
    ...
    setCompetitorBranchInput({ ...competitorBranchInput, [action.name]: value });
    if (value === "" || value === null) {
  ...
  const submitHandler =async (e) => {
    ...
    if(data.compId!==null && data.compNo!==null && data.country!==null && data.state!==null && data.district!==null  && data.city!==null)
...
const deleteBranch =async (id) => {
  ...
  return (
    ...
    <div className="card-body ">
      ...
      <div>
        {isMounted && <Loader size="lg" backdrop content="Fetching Data..." />}
      </div>
          ...
          {branchList.map((item, index) => {
            return (
              <tr key={item.id}>
                ...
                <td> {item.city} </td>
                <td>
                <Link to = {`competitorbranch/${item.id}`}><i className="fas fa-edit text-primary"  ></i></Link>
                <Link onClick={() => deleteBranch(item.id, item.city)}><i className="fas fa-trash text-danger mx-3"></i></Link>
                </td>
              </tr>
            );
          })}
