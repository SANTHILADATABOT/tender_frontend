...
const UploadDoc = () => {
  ...
  const postData = (data) => {
    axios.post(`${baseUrl}/api/communicationfiles/docupload`, data, config).then((resp) => {
  ...
  const putData = (data,UploadDocId ) => {
    ...
    axios.post(`${baseUrl}/api/communicationfiles/docupload/${UploadDocId}`, data, config).then((resp) => {
      if (resp.data.status === 200){
        ref.current.getDocList()
        resetform()
        toastSuccess(resp.data.message)
