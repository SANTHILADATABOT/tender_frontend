...
const LetterOfAcceptance = () => {
  ...
  const { letterofacceptence: filepath } = useImageStoragePath();
const [pdfFile, setPdfFile]=useState("");
  ...
  var setletterofacceptanceForm = (response) => {
    ...
    mediumSelectValue(options.find(
      (x) => x.value === data.medium_select
    ))
    
    var imgUrl =filepath+data.wofile; 
          ...
          showpreviewOnLoad(imgUrl,response.data.id);
    }
  };
  ...
  const putData = (data,lettacpId ) => {
    axios.post(`${baseUrl}/api/letteracceptance/creation/update/${lettacpId}`, data, config).then((ressp) => {
        ...
        <div className="row align-items-center ">
          ...
          <div className="inputgroup col-lg-6 mb-4">
            <div className="row align-items-center font-weight-bold">
              ...
              <div className="col-lg-8">
                <LetterAcceptanceDoc file={file}  pdfFile={pdfFile!=="" ? pdfFile : undefined} />
              </div>
            </div>
          </div>
        </div>
