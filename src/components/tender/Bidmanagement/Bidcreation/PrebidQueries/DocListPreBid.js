...
import DocCardPrebid from "./DocCardPrebid";

const DocListPreBid = forwardRef((props, ref) => {
            ...
            <PreLoader loading={loading}>
                {docList.length > 0  && <div className={`overflow-auto col-lg-12 ${styles.doclist}`}>
                    {docList.map((item, index) => {
                        return(
                            <DocCardPrebid key = {index} item = {item} index={index} onDelete={deleteHandler} onEdit={props.onEdit}/>
                        )
                    })}
                </div>}
                {docList.length <= 0 && 
